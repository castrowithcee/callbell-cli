...
data as JSON. `invoke` takes its arguments either as repeated `--arg name=value`, typed by the input
schema, or as one JSON object on stdin for anything nested; it writes a JSON result.

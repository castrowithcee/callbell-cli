...
func transportError(op string, err error) error {
	...
	case errors.Is(err, context.DeadlineExceeded), errors.As(err, &netErr) && netErr.Timeout():
		return &provider.Error{Class: provider.ClassTimeout, Op: op, Message: "Telegram did not answer in time"}
	...
	default:
		return &provider.Error{Class: provider.ClassUnreachable, Op: op, Message: "Telegram could not be reached"}
	}
}
